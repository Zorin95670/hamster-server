...
public class DatePredicateFilter extends PredicateFilter {
    ...
    public final boolean extract() {
        ...
        for (int index = 0; index < this.getValues().length; index++) {
            ...
            if (!matcher.find()) {
                // TODO: throw new GlobalException(ErrorType.WRONG_FILTER_VALUE, this.getName(), value);
            }

            String operator = matcher.group(OPERATOR_POSITION);
            if (operator != null) {
                this.setOperator(index, PredicateOperator.get(operator).orElseThrow(() -> new RuntimeException()));
                        // TODO: new GlobalException(ErrorType.WRONG_FILTER_OPERATOR, this.getName(), operator)));
            }
            ...
            if (PredicateOperator.BETWEEN.equals(this.getOperator(index))
                ...
                // TODO: throw new GlobalException(ErrorType.EMPTY_VALUE, this.getName());
            }
        }
        return true;
    }
